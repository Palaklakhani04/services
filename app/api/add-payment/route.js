...
export async function POST(req) {
    ...
    const { userId, paymentMode, transactionId, response, amount, packageId, status, packageTime, bookingDate } = await req.json();


    console.log(userId, paymentMode, transactionId, response, amount, packageId, status, packageTime, bookingDate, 'add payment data------')


    try {
        await dbConnect();

        const results = await new Payment({
